...
import createLogger from '@salesforce/apex/createLogger.createLogger';  
...
export default class Createnewaccountform extends LightningElement {
  ...
  handleShowAccountSearch(event) {
    const customEvent = new CustomEvent("showsearchstep", {
        detail: {accountName: this.accountName, phone: this.phone, stateName: this.stateName, cityName: this.cityName,
            zipcode: this.zipcode, countryName:this.countryName}
    });
...
handleError(error, errorMessage) {
  ...
  if (error.body != null && error.body.output != null && error.body.output.errors != null && typeof error.body.output.errors === 'object' && error.body.output.errors.length > 0) {
